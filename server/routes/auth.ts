    ...
    req.session.regenerate((err) => {
      ...
      req.session.save((err) => {
        ...
        res.json({
          ...
          role: user.role, language: user.language, region: user.region,
          district: user.district, farm_size_acres: user.farm_size_acres,
        });
      });
    });
  } catch {
...
router.put('/profile', isAuthenticated, async (req, res) => {
  try {
    ...
    await dbRun(
      ...
      region || null, district || null, farm_size_acres || null, req.session.userId!
    );
    const user = await dbGet(
      'SELECT id, email, phone_number, first_name, last_name, role, language, region, district, farm_size_acres, created_at FROM users WHERE id = ?',
      req.session.userId!
...
router.put('/password', isAuthenticated, async (req, res) => {
  try {
    ...
    await dbRun(
      ...
      bcrypt.hashSync(new_password, 10), req.session.userId!
    );
...
router.get('/users', isAdmin, async (req, res) => {
  const users = await dbAll('SELECT id, email, phone_number, first_name, last_name, role, is_active, created_at FROM users');
  res.json(users);
});
